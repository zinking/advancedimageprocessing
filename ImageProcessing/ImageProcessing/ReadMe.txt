...
ImageProcessing.h
这是应用程序的主要头文件。它包括其他项目特定的头文件(包括 Resource.h)，并声明 CDIPApp 应用程序类。

ImageProcessing.cpp
这是包含应用程序类 CDIPApp 的主要应用程序源文件。
...
res\Toolbar.bmp
此位图文件用于为工具栏创建平铺图像。
初始工具栏和状态栏在 CMainFrame 类中构造。使用资源编辑器编辑此工具栏位图，并更新 ImageProcessing.rc 中的 IDR_MAINFRAME TOOLBAR 数组以添加工具栏按钮。
/////////////////////////////////////////////////////////////////////////////

应用程序向导创建一个文档类型和一个视图:

DIPDoc.h，DIPDoc.cpp - 文档
这些文件包含 CDIPDoc 类。编辑这些文件可以添加特殊文档数据并可实现文件保存和加载(通过 CDIPDoc::Serialize)。

DIPView.h，DIPView.cpp - 文档的视图
这些文件包含 CDIPView 类。
CDIPView 对象用于查看 CDIPDoc 对象。
